...
{
    private string RoomDifficulty, RoomName, RoomPassword, JoinedRoom;
    private Text Log;
    ...
    public void OnClick_CreateNewRoom()
    {
        ...
        else
        {
            ...
            if (PhotonNetwork.CreateRoom(RoomName, NewGameOptions))
            {
                ...
                JoinedRoom = RoomName;
                GameObject.Find("RoomCreationCanvas").GetComponent<Transform>().localScale = Vector3.zero;
                GameObject.Find("Scene").GetComponent<ChangeScene>().GoToScene("WaitingRoom");
            }
            else
            {
    ...
    {
        Log.text = "You joined \"" + JoinedRoom + "\".";
        Log.color = Color.green;
    }
}
